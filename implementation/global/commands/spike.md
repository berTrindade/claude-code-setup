...
Investigate the question provided in the arguments (e.g. `/spike how does auth work` or `/spike should we use React Query for data fetching`).
...
Use Grep, Glob, and Read to explore the codebase. Trace the relevant data flow from entry point to output.
...
No implementation. If a plan is needed after the spike, use `/plan`.
